...
void mergeNotes(char* args[], int numArgs) {
	...
	result = checkSame(firstNote, defaultNote);
	if (result == -1) return;

	result = checkSame(secondNote, defaultNote);
	if (result == -1) return;
...
void duplicateNote(char* args[], int numArgs) {
	...
	char* defaultNote = "default";	
	result = checkSame(existingNote, defaultNote);
	if (result == -1) return;
...
void importNote(char* args[], int numArgs) {
	...
	char* defaultNote = "default";	
	result = checkSame(import, defaultNote);
	if (result == -1) return;
...
void archiveCurrent() {
	...
	char* defaultNote = "default";	
	int result = checkSame(currentNoteName, defaultNote);
	if (result == -1) return;
...
void archiveNote(char* args[], int numArgs) {
	...
	// prevent the user from archiving default
	char* defaultNote = "default";	
	int result = checkSame(noteToArchive, defaultNote);
	if (result == -1) return;
...
void renameNote(char* args[], int numArgs) {
	...
	char* defaultNote = "default";
	result = checkSame(oldName, defaultNote);
	if (result == -1) return;
